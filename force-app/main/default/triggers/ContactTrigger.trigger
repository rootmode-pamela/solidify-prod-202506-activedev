trigger ContactTrigger on Contact (after undelete, before delete, before insert, before update, after update) {
    
    if(trigger.isBefore && trigger.isUpdate) {
  /*  Ill-advised attempt to add proposal/scenario info to contact.
       ContactTriggerHandler.calcRate(trigger.new);
        ContactTriggerHandler.calcRate2(trigger.new);
        ContactTriggerHandler.calcRate3(trigger.new);
        ContactTriggerHandler.calcRate(trigger.new);
        ContactTriggerHandler.calcRate2(trigger.new);
        ContactTriggerHandler.calcRate3(trigger.new);  */
    }
